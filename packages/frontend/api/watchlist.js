...
export default async function handler(req, res) {
  ...
  const { data, error } = await supabase.from('equity_bars').select('symbol, close, rsi_14, stoch_k, williams_r, volume_ratio, sma_20, vwap').eq('bar_date', latestRow.bar_date).not('rsi_14','is',null).order('rsi_14', { ascending: true })
