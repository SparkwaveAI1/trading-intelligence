...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabase.from('paper_trades').insert({ signal_event_id: signal_event_id ?? null, trade_type: 'paper', market_type: market_type ?? 'equity', symbol: symbol ?? null, direction, entry_price, stop_level: stop_level ?? null, target_level: target_level ?? null, size_pct: size_pct ?? null, notes: notes ?? null, outcome: 'open' }).select().single()
    if (error) return res.status(500).json({ error: error.message })
